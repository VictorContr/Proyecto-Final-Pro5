...
export class ProjectileDonMedusa extends BolaAzul_vc_jc.Projectile {
    ...
    static entitiesThatCollide = BolaAzul_vc_jc.Ayudante_vc_jc.obtenerIdsEntidades_vc_jc(
        'Alma', 'Block', 'Medusa', 'Egg', 'Gol', 'Leeper', 
        'DonMedusa', 'Player', 'Rocky', 'Skull', 'Snakey', 'Cofre_vc_jc', 
        'PuertaCerrada_vc_jc', 'DoorOpened', 'Heart'
    );

    constructor(shooter, direction) {
        if (!new.target) {
            throw new TypeError("Class constructor ProjectileDonMedusa cannot be invoked without 'new'");
        }
        
        super(shooter, direction, 'mobSprites', 8);

        // Selección de frame y ángulo según dirección
        switch (direction) {
            case Phaser.Tilemap.NORTH:
                this.frameName = 'projectileDonMedusa1';
                this.angle = 0;
                break;
            case Phaser.Tilemap.EAST:
                this.frameName = 'projectileDonMedusa2';
                this.angle = 0;
                break;
    ...
    static canTarget(shooter, target) {
        ...
        for (let i = 0; i < cellsToCheck.length; i++) {
            ...
            if (tile && ProjectileDonMedusa.tilesThatCollide.includes(tile.index)) {
                return -1;
            }
            ...
            const collidingEntities = BolaAzul_vc_jc.Ayudante_vc_jc.obtenerEntidadesDesdeIndices_vc_jc(
                ProjectileDonMedusa.entitiesThatCollide, 
                entities
            );
    ...
    static shootTo(shooter, target) {
        ...
        const direction = ProjectileDonMedusa.canTarget(shooter, target);
        
        if (direction >= 0) {
            return new ProjectileDonMedusa(shooter, direction);
        }
...
BolaAzul_vc_jc.ProjectileDonMedusa = ProjectileDonMedusa;
