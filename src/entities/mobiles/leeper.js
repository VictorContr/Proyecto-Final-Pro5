...
export class Leeper extends BolaAzul_vc_jc.Mobile {
    ...
    constructor(game, x, y) {
        ...
        if (!new.target) throw new TypeError("Class constructor Leeper cannot be invoked without 'new'");
        
        super(game, x, y, 'mobSprites', 'leeperDown1', {
            gid: BolaAzul_vc_jc.Global_vc_jc.Entities.Leeper
        });
        ...
        if (!this.level) {
            console.error("Leeper created without level reference");
            return;
        }
    ...
    nextAction() {
        if (!this.isAwaken || this.isSleeping || !this.level?.player) return;
